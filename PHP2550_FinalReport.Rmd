...
#create week variable for the overall dataset
...
```{r, echo = FALSE}
#create time series data for overall cases
...
```{r, echo = FALSE}
#get cluster variable covariate data
...
```{r, echo = FALSE}
#fit poisson model with only SNP clusters covariates
...
```{r, echo = FALSE}
#MSE function for 1 observation
...
#fit poisson cluster variable interacting with season model
...
```{r, echo = FALSE}
#get isolate sources covariate data
...
#fit poisson isolate source model
...
#fit poisson isolate source interacting with season model
...
#fit poisson cluster + isolates model
...
#fit poisson model with cluster and source variables interacting with season
...
```{r, echo = FALSE}
#get table depicting AIC and MSE for each model
...
```{r, echo = FALSE}
#old code to get time series forecasting
...
```{r, echo = FALSE}
#new MSE calculator function for 2 observations
...
```{r, echo = FALSE}
#code for getting table of AIC and MSE for 2 observation models 
models <- c("Cluster Variables", "Cluster with Seasons", "Source Variables", "Source with Seasons", "Cluster and Source", "Cluster, Source, and Seasons")
...
```{r, echo = FALSE}
#MSE calculator function for 3 observations
...
```{r, echo = FALSE}
#table for checking AIC and MSE of all models with 1, 2, and 3 observations
models <- c("Cluster Variables", "Cluster with Seasons", "Source Variables", "Source with Seasons", "Cluster and Source", "Cluster, Source, and Seasons")
...
```{r, echo = FALSE}
#get frequencies for weekly cases (50 is highest frequency, then 55)
#sort(table(overall_ts))
```

```{r, echo = FALSE}
#MSE calculator function for 1 observation and 1 conditional mean autoregressed on
...
```{r, echo = FALSE}
#fit base negative binomial model
...
```{r, echo = FALSE}
#fit negative binomial model with initial conditional mean 50
...
```{r, echo = FALSE}
#fit negative binomial model with initial conditional mean 55
...
```{r, echo = FALSE}
#fit negative binomial model with initial conditional mean as the rounded average weekly cases
...
```{r, echo = FALSE}
#new table to show AIC and MSE of best poisson model and negative binomial models
...
```{r, echo = FALSE}
#final models chosen for forecasting
...
#forecast plot
set.seed(10)
#simulate forecast trajectories
...
ggplot(ts_final2, aes(x = Weeks, y = Cases, color = Models, groups = Models)) + 
  ...
  scale_x_continuous(name = "Time (in Weeks)", breaks = c(1, 53, 105, 157, 209, 261, 313, 365), 
                     labels = c("2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021"))
#if we wanted to save our plot as a png file
...
```{r, echo = FALSE}
#get negative binomial model summary
